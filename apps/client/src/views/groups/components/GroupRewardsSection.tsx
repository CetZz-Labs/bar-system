...
import { Gift, Coins, MapPinOff, Loader2, XCircle } from "lucide-react";
...
/**
 * LB-111: pieza "montable" extraída de `GroupRewardsView.tsx` (LB-68/LB-72)
 * para no duplicar la lógica de recompensas + canje entre esa vista y la
 * vista Salida nueva (`GroupOutingView.tsx`). Encapsula sus propias queries
 * (salida activa, recompensas, canjes) y mutaciones (crear/cancelar canje) —
 * el único estado que recibe de afuera es `groupId` / `canManageRedemptions`,
 * ya resueltos por el caller a partir de su propio fetch de grupo.
 */
...
export interface GroupRewardsSectionProps {
  groupId: string | undefined;
  canManageRedemptions: boolean;
  /** Acción del botón/enlace "Volver" en los estados vacío/error de esta sección. */
  onBack: () => void;
}

export function GroupRewardsSection({
  groupId,
  canManageRedemptions,
  onBack,
}: GroupRewardsSectionProps) {
  ...
  // Saldo global (points_balance_updated) no aplica acá; se ignora.
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      {isLoading && <Spinner center size="lg" label="Cargando recompensas" />}
