 ...
 *
 * LB-61 debe invocar esta función en el momento exacto en que confirma el
 * primer `Consumption` de una salida (transición a `ConsumptionStatus.CONFIRMED`),
 * pasándole el `outingId` de esa salida (como string, `ObjectId` también sirve
 * porque Mongoose lo castea). No hace falta que LB-61 chequee de antemano si
 * ya se acreditó: la función es **idempotente por salida** vía el flag
 * `Outing.attendancePointsAwarded`.
 *
 * Comportamiento:
 ...
 * 3. Si `outing.attendancePointsSnapshot` (tomado en
 *    `OutingController.createOuting`, ver comentario ahí) es 0: NO acredita
 *    nada, NO crea ningún `PointsTransaction` (silencio total, por spec), pero
 *    SÍ marca `attendancePointsAwarded = true` para no volver a evaluarla.
 *    Devuelve `{ awarded: false }`.
 ...
 *    (`type: ATTENDANCE`) con
 *    `label: "Asistencia — {bar.name} ({día de bar de outing.scheduledFor, dd/mm/aaaa})"`.
...
export async function awardAttendancePointsIfFirst(
    ...
    try {
        ...
        const amount = outing.attendancePointsSnapshot;
        ...
        await PointsTransaction.create([{
            ...
            amount,
            label: `Asistencia — ${bar?.name ?? 'el bar'} (${formatBarDayLabel(outing.scheduledFor, bar?.closingTime)})`,
        }], { session });
...
/** "dd/mm/aaaa" del día de bar (no del calendario) al que pertenece `scheduledFor`. */
function formatBarDayLabel(scheduledFor: Date, closingHour = '06:00'): string {
    const { start } = getBarDayRange(scheduledFor, closingHour);
