...
- Tarea sobre `apps/server` (Node/Express/Mongoose) → `.claude/rules/backend.md`
- Tarea sobre `apps/client` (React/Vite) → `.claude/rules/frontend.md`
...
3. Si te bloqueás (herramienta falla, ambigüedad en el alcance, conflicto con
   `.claude/rules/`), no improvises un workaround: documentá el bloqueo en el
